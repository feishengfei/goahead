...
static int showNatAdvanceRulesASP(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(i=0;i<NAT_ADVANCE_NUM;i++){
        ...
        //printf("extport_end=%s\t",tmp);
			...
			//Inc S
			websWrite(wp, T("<td><center><span class=\"table_font\"><input name=\"PortRule_IncS%d\" size=10 maxlength=31 value=\"%s\" type=\"text\"></span></center></td>"),i+1,atoi(InportS)==0?"":InportS);
			//Inc E
			websWrite(wp, T("<td><center><span class=\"table_font\"><input name=\"PortRule_IncE%d\" size=10 maxlength=31 value=\"%s\" type=\"text\"></span></center></td>"),i+1,atoi(InportE)==0?"":InportE);
			//Tri S
			websWrite(wp, T("<td><center><span class=\"table_font\"><input name=\"PortRule_TriS%d\" size=10 maxlength=31 value=\"%s\" type=\"text\"></span></center></td>"),i+1,atoi(TriportS)==0?"":TriportS);
			//Tri E
			websWrite(wp, T("<td><center><span class=\"table_right\"><input name=\"PortRule_TriE%d\" size=10 maxlength=31 value=\"%s\" type=\"text\"></span></center></td>"),i+1,atoi(TriportE)==0?"":TriportE);						
			websWrite(wp, T("</tr>\n"));	
