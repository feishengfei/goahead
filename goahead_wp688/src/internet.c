...
#define SYS_PORT_LIST_PATH "/tmp/socket_port_list.tmp"
	...
	system("netstat -l | awk '{print $4}' | awk 'BEGIN{FS=\":\"}{if(length($2)) print $2;}' > /tmp/socket_port_list.tmp ");
...
{
	char *value;
	value = nvram_safe_get("wan0_proto");
        return websWrite(wp, T("%s"), value);
}
...
static int getWlan5GMac(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    ezplib_get_attr_val("wl1_ifname_rule", 0, "ifname", tmp2, SHORT_BUF_LEN,
                EZPLIB_USE_CLI);
 ...
 *  Ricardo correct it  for bug4336  2013-03-22
 */
int getIndexOfStaticRoute(char *dest, char *netmask)
{
	char buf_dest[TMP_LEN], buf_mask[TMP_LEN];
	int i, mask, rule_num = atoi(nvram_safe_get("rt_rule_num"));

	netmask = mask2prefix(netmask);
	if(!strcmp(netmask, "") || !strcmp(netmask, "0"))
		return -1;
	...
	for(i=0; i<rule_num; i++)
	{
		...
		mask = min(atoi(netmask), atoi(buf_mask));
		netmask = prefix2mask(mask);
		if( (inet_addr(dest) & inet_addr(netmask)) == (inet_addr(buf_dest) & inet_addr(netmask)))
			return i;
	}
	return -1;
}
...
static void addRouting(webs_t wp, char_t *path, char_t *query)
{
	...
	if(!strcmp(interface, "WAN"))
	{
		...
		if (!strcmp(TempBuf, "normal"))
			...
			ezplib_get_attr_val("wl_wisp_mode_rule", 0, "mode2.4g", TempBuf, SHORT_BUF_LEN, EZPLIB_USE_CLI);
			if(!strcmp(TempBuf, "wisp"))
				true_interface = "apclii0";
			else
...
static void setStp(webs_t wp, char_t *path, char_t *query)
{
    ...
    if(!strcmp(enable, "1"))
    { 
    	snprintf(tmp, TMP_LEN, "brctl stp br0 on");
    }else{

	snprintf(tmp, TMP_LEN, "brctl stp br0 off");
    }
...
static void setLanIP(webs_t wp, char_t *path, char_t *query)
{
		...
		for (i = 0; i < num; i++) {
			...
			if (!strcmp(buf, "normal") || !strcmp(buf, "wisp") || !strcmp(buf, "wisp_ur"))
			{
			...
			else if (!strcmp(buf, "ap") || !strcmp(buf, "ur"))
			{
				...
				if(!strcmp(lanIp_mode, "0")) //from dhcp 
				{
					...
					//lan0_static_ip = websGetVar(wp, T("lanIpApMode"), T(""));
					//lan0_static_mask = websGetVar(wp, T("lanNetmaskApMode"), T(""));
					//lan0_static_gateway = websGetVar(wp, T("lanGatewayApMode"), T(""));
					printf("lan_fallback_ip: %s, lan_fallback_mask: %s\n", lan_fallback_ip, lan_fallback_mask);
					...
					//ezplib_replace_attr("lan_static_rule", i ,"ipaddr_ap", lan0_static_ip);
					//lan0_static_mask = mask2prefix(lan0_static_mask);
					//ezplib_replace_attr("lan_static_rule", i ,"mask_ap", lan0_static_mask);
					/* Ended by Bruce Liu */
				}
...
#if 1
		if (!strcmp(buf, "ap") || !strcmp(buf, "ur")) {
			system("kill  `pidof udhcpc`");
		}
		system("ifup lan0 &");
#else
...
{    
        ...
        */
        ...
        for(j=1; j<=dhcp_static_lease_num; j++)
        {
            ...
            char mycmd[100];
            memset(mycmd, 0, sizeof(mycmd));
			snprintf(mycmd, sizeof(mycmd), "cat /var/dhcp.leases |grep -v %s>/var/dhcp.leases", dhcpStaticIp);	
			printf("cmd=%s\n", mycmd);	
			system(mycmd);
...
//Liteon Modified, restart Wan >>> 
	...
	//kill process: udhcpc, it send release packet to dhcp server.
	system("kill  `pidof udhcpc` "); // Modified by Bruce Liu, 2013-3-28.
	system("ifdown wan0");
	system("kill -9 `pidof l2tpd` ");
	system("kill -9 `pidof pptp` ");
	system("kill -9 `pidof pppd` ");
	system("rm /var/run/l2tp.sock");
	system("rm /var/run/pptp -rf");
	system("rm /var/run/ppp*.pid");
	system("rm /var/run/wan0.pid");
	system("sleep 3 && ifup wan0 &");
...
ad_tools_parse_ping_result(char *act, char *res, char *output)
{
    ...
    if (!strcmp(act, "ping")){
        ...
        sprintf(output, "<p>%s : %d<br/>%s : %d<br/>%s : %d %s<br/></p>", "Packet Sent", npktx,"Packet Received", npktr, 
...
{     
     ...
     char_t * ro_username, *ro_passwd, *rw_username, *rw_passwd;
	 char_t *sshenable,*sshport,*telnetenable,*telnetport,*value,*httpsenable,*httpsport,*valueport;
     ...
     ro_username = websGetVar(wp, T("snmp_user_ro"), T(""));
     ...
     printf("\nro_username=%s\nro_passwd=%s\nrw_username=%s\nrw_passwd=%s\n",
		     ro_username,ro_passwd,rw_username,rw_passwd);
#endif
     ...
     if( atoi(enable) == 1 ) {
          ...
          if( atoi(usm_enable) == 1 ) {
               nvram_set("snmpd_ro_username", ro_username);
	       nvram_set("snmpd_ro_passwd", ro_passwd);
               nvram_set("snmpd_rw_username", rw_username);
	       nvram_set("snmpd_rw_passwd", rw_passwd);
/*
               ezplib_replace_attr("snmpd_user_rule", 0, "ro_username", ro_username);
               ezplib_replace_attr("snmpd_user_rule", 0, "ro_passwd", ro_passwd);
               ezplib_replace_attr("snmpd_user_rule", 0, "rw_username", rw_username);
               ezplib_replace_attr("snmpd_user_rule", 0, "rw_passwd", rw_passwd);
*/
          }      
          //ezplib_replace_attr("snmpd_user_rule", 0, "contact", contact);
	  nvram_set("psyscontact",contact);
          if( atoi( v1_enable) || atoi( v2c_enable) ) {
	       ...
	       /*
               ezplib_replace_attr("snmpd_com2sec_rule", 0, "community", comm_ro);
               ezplib_replace_attr("snmpd_com2sec_rule", 1, "community", comm_rw);
	       */
          }
     }
	 memset(cmd,0,sizeof(cmd));
	 ...
	 if(atoi(telnetenable) == 1)
	 {
	 	if(atoi(value)==1)
	 	{
			if(atoi(valueport) != atoi(telnetport))
			{
				sprintf(cmd,"killall telnetd && /usr/sbin/telnetd -p %d",atoi(telnetport));
				system(cmd);
			}
	 	}
		else
		{
			sprintf(cmd,"/usr/sbin/telnetd -p %d",atoi(telnetport));
	 		system(cmd);
		}
	 }
	 else
	 ...
	 valueport=nvram_safe_get("https_port");
