 ...
 if (set_flag==0)//Login
 {
	...
	{	
		...
		if (!strcmp(loginpass, "qweasdzxc"))
		{
				...
				{
					if (!strcmp(admpass, "1234") && !strcmp(uname, "admin") && !strcmp(first_config, "1"))
					{
...
static void logsetting(webs_t wp, char_t *path, char_t *query)
{
	char *SystemMaintenance, *logdns, *PPP, *UPnP, *WLAN, *NTPClient, *SYSwarning, *select_num;
       ...
       value = nvram_get("log_index");
       system("/sbin/syslog-ng-init stop");
	...
	//snprintf(tmp, TMP_LEN, "/sbin/syslog-ng-init start");
       system("/sbin/syslog-ng-init start");
...
static	long long WLAN1RXBytesPerSec;
static	long long APCLIRXBytesPerSec;
static	long long LANTXBytesPerSec;
static	long long WANTXBytesPerSec;
static	long long WLANTXBytesPerSec;
static	long long WLAN1TXBytesPerSec;
static	long long APCLITXBytesPerSec;
...
int GetIfBytesPerSecASP(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	//WLAN
	WLANRXBytes0s = getIfStatistic( "rai0", RXBYTE);
	WLANTXBytes0s = getIfStatistic( "rai0", TXBYTE);	
	WLAN1RXBytes0s = getIfStatistic( "ra0", RXBYTE);
	WLAN1TXBytes0s = getIfStatistic( "ra0", TXBYTE);

	APCLIRXBytes0s = getIfStatistic("apclii0", RXBYTE);
	APCLITXBytes0s = getIfStatistic("apclii0", TXBYTE);
	...
	//WLAN
	WLANRXBytes1s = getIfStatistic( "rai0", RXBYTE);
	WLANTXBytes1s = getIfStatistic( "rai0", TXBYTE);	
	WLAN1RXBytes1s = getIfStatistic( "ra0", RXBYTE);
	WLAN1TXBytes1s = getIfStatistic( "ra0", TXBYTE);

	APCLIRXBytes1s = getIfStatistic("apclii0", RXBYTE);
	APCLITXBytes1s = getIfStatistic("apclii0", TXBYTE);
	...
	WLAN1RXBytesPerSec = WLAN1RXBytes1s - WLAN1RXBytes0s;
	APCLIRXBytesPerSec = APCLIRXBytes1s -APCLIRXBytes0s;
	...
	WLAN1TXBytesPerSec = WLAN1TXBytes1s - WLAN1TXBytes0s;
	APCLITXBytesPerSec = APCLITXBytes1s - APCLITXBytes0s;
	websWrite(wp, T("OK"));
	return 0;	
}
...
int getWLANRxByteASP(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	snprintf(buf, 32, "%lld", WLANRXBytesPerSec + APCLIRXBytesPerSec);
	websWrite(wp, T("%s"), buf);
	return 0;
#else
	FILE *fwlanlink;
...
{
	char_t buf[32];
	long long data = getIfStatistic( "rai0", RXPACKET);
	long long apcli_data = getIfStatistic( "apclii0", RXPACKET);
	snprintf(buf, 32, "%lld", data + apcli_data);	
	websWrite(wp, T("%s"), buf);
	return 0;
}
...
int getWLANTxByteASP(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	snprintf(buf, 32, "%lld", WLANTXBytesPerSec + APCLITXBytesPerSec);
	websWrite(wp, T("%s"), buf);
	return 0;
#else
	FILE *fwlanlink;
...
{
	char_t buf[32];
	long long data = getIfStatistic( "rai0", TXPACKET);
	long long apcli_data = getIfStatistic( "apclii0", TXPACKET);
	snprintf(buf, 32, "%lld", data + apcli_data);	
	websWrite(wp, T("%s"),buf);
	return 0;
}
...
{
	char_t buf[32];
	long long data = getIfStatistic( "rai0", COLLS);
	snprintf(buf, 32, "%lld", data);	
	websWrite(wp, T("%s"),buf);
	return 0;	
}
...
int getLANLinkStatus(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    //char log[50];
    // sprintf(log, "echo %d %s>/dev/console", argc, argv[1]);
    
    //system(log);
    num = atoi(field);
    char cmd[50];
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "/sbin/ezp-portstate %d link > /tmp/lanlink", num);
    printf("cmd = %s\n", cmd);
    system(cmd);
    fwanlink = fopen("/tmp/lanlink", "r");
    fscanf(fwanlink,"%s",wanlink);
    fclose(fwanlink);
    system("rm -f /tmp/lanlink");
    
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "/sbin/ezp-portstate %d speed > /tmp/lanspeed", num);
    system(cmd);
    fwanspeed = fopen("/tmp/lanspeed", "r");
    fscanf(fwanspeed,"%s",wanspeed);
    fclose(fwanspeed);
    system("rm -f /tmp/lanspeed");
...
int getWLANLinkRate(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	//system("iwconfig rai0 | grep 'Rate' | sed 's/^.*Rate\=//g' | sed 's/\ .*$//g' > /tmp/link_rate");
	ezplib_get_attr_val("wl_mode_rule", 0, "mode", TempBuf, 32, EZPLIB_USE_CLI);
	if (!strcmp(TempBuf, "ap"))
	{
		system("/sbin/ezp-wps-set 1 0 0 0");//rai0
		if (NULL == (fp = fopen("/tmp/link_rate", "r")))
		{
	         websWrite(wp, T("300M / 300M"));
	         return 0;
		}
		fscanf(fp, "%s", TempBuf);
		fclose(fp);
		...
		if (strlen(TempBuf)>0)
		{
			websWrite(wp, T("%sM / %sM"), TempBuf,TempBuf);
		}else{
        	websWrite(wp, T("300M / 300M"));
		}
		return 0;
	...
	{
		//system("/sbin/ezp-wps-set 1 00 0 0");//00 stand for apclii0
		system("iwpriv apclii0 stat | grep 'Current RxTx Rate' | awk -F ',' '{print $2}' > /tmp/link_RxRate");
		system("iwpriv apclii0 stat | grep 'Current RxTx Rate' | awk -F ',' '{print $3}' > /tmp/link_TxRate");
		if (NULL == (fp = fopen("/tmp/link_RxRate", "r")))
		{
	         websWrite(wp, T("-- / --"));
	         return 0;
		}
		fscanf(fp, "%s", TempBuf);
		fclose(fp);
		system("rm -f /tmp/link_RxRate");

		if (NULL == (fp = fopen("/tmp/link_TxRate", "r")))
		{
	         websWrite(wp, T("-- / --"));
	         return 0;
		}
		memset(TempBuf2, 0 , sizeof(TempBuf2));
		fscanf(fp, "%s", TempBuf2);
		fclose(fp);
		system("rm -f /tmp/link_TxRate");
		if (strlen(TempBuf)>0 && strlen(TempBuf2)>0)
		{
			websWrite(wp, T("%s / %s"), TempBuf,TempBuf2);
		}else{
	        websWrite(wp, T("-- / --"));
		}
		return 0;
...
int loglist(int eid, webs_t wp, int argc, char_t **argv)//Gordon add for systemlog 20081230
{
  ...
  int show_flag=0;//when select show_all,0 not show log 1 show log
    ...
    ezplib_get_attr_val("log_rule", 0, "dhcp_serv", dhcp_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "dhcp_cli", dhcpcli_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "web_mgmt", web_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "dns", dns_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "ppp", ppp_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "upnp", upnp_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "ddns", ddns_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "sys_warning", sys_buf, 32, EZPLIB_USE_CLI);
    ezplib_get_attr_val("log_rule", 0, "ntp", ntp_buf, 32, EZPLIB_USE_CLI);
	...
	for(charnum=0;charnum<16384;charnum++)
	{
	 if((log1[charnum]==00)&&(log1[charnum+1]==00)&&(log1[charnum+2]==00)&&(log1[charnum+3]==00))
	  ...
	  if(!strcmp(LOG_index,"show_all"))//ALL LOG
	  {
	     print_index=1;
          //<web>
	    if((log1[charnum]==60)&&(log1[charnum+1]==87)&&(log1[charnum+2]==69)&&(log1[charnum+3]==66)&&(log1[charnum+4]==62)&&!strcmp(web_buf, "0"))
	    {
	       print_index=0;
	    }
	   //<dns>
...
static int getIPv6LanAddr(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if((fp = fopen("/etc/br0ipv6", "r")) != NULL)
	{
