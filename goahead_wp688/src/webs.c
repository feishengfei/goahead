...
#define LINUX_PARTSIZE  16318464 //Linux part size
/*********************************** Code *************************************/
/*
 *	Open the GoAhead WebServer
 */

int websOpenServer(int port, int retries,int usehttps)
{
	websMimeType	*mt;
...
//Steve patched 2010/05/26
	if(len_aa>LINUX_PARTSIZE)
       {
       	printf("%s => dont_write_to_ram==>len_aa > 16318464\n", __FUNCTION__);
	len_aa=0;
        dont_write_to_ram=0;
         ...
         if (wp->clen > 0)
         {
			   ...
			   websSetVar(wp, T("CONTENT_LENGTH"), value);
         	   ...
         	   printf("\n%s => wp->clen=%d\n", __FUNCTION__, wp->clen);
                        if((wp->clen)>LINUX_PARTSIZE)
			   {		   
			   		printf("%s => wp->clen > 16318464\n", __FUNCTION__);
