...
static int			usehttps=0;						/* https flag */
static int			httpsport=0;					/* https port */
...
static int initWebs()
{
	...
	/* Added by Bruce Liu, 2013-03-27, In bridge mode, if LAN dynamic obtain ip fail, after reboot DUT, goahead not start */
	if (!strcmp(lan_ip, "")) {
		char *value;
		char buf[256];
		memset(buf, 0, sizeof(buf) );
		ezplib_get_attr_val("lan_static_rule", 0, "ipaddr", buf, 256, 0);	
		value = buf;
		lan_ip = value;
	}
	printf("ip: %s\n", lan_ip);
 ...
 */
	websOpenServer(port, retries,usehttps);
