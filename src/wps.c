...
{
	char buf[65], *ptmp1, *ptmp2;
	FILE *fp = NULL;
	int len=0, i=0;
	char TempBuf[128], cmd[128];
    char_t *wr_itf;
    char essid[33];
#if 0 /* Modified by Ricardo 2013-03-21 */
    #define ap_client_essid "/tmp/ap_client_essid"
	//system("iwconfig apclii0 | grep 'ESSID' | sed 's/^.*ESSID:\"//g' | sed 's/\".*$//g' > /tmp/wisp_essid_name");
     if (ejArgs(argc, argv, T("%s"), &wr_itf) < 1) {
            system("/sbin/ezp-wps-set 1 00 3 0");
    ...
    sprintf(cmd, "iwconfig %s | grep ESSID | awk '{print $4}' | cut -c 8-42  > %s", wr_itf, ap_client_essid);

    system(cmd);

    if (NULL == (fp = fopen(ap_client_essid, "r"))){
        		return 0;
    }
    ...
    if(!ptmp2) {
        websWrite(wp, T("Not-Interface"));
        return 0;
    } else if (ptmp2 == ptmp1) {
        websWrite(wp, T("Not-Associated"));
        return 0;
    } else {
        while (ptmp1 != ptmp2 && ptmp1 != NULL && i < 32) {
            essid[i] = *ptmp1++;
            i++;
        }
        essid[i] = '\0';
...
static int getWISPapcli0ConnectStatus(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	//system("iwconfig apclii0 | grep 'Access Point:' | sed 's/^.*Access Point: //g' > /tmp/wisp_essid_mac");
